  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        ...
        console.warn(
          `Network request failed, retrying... (${attempt}/${maxRetries})`,
        );
...
const handleAuthError = (
  ...
  switch (error.code) {
    case 'auth/network-request-failed':
      if (!networkConnected) {
        return new Error(
          'You appear to be offline. Please check your internet connection and try again, or use Demo Mode to explore the app offline.',
        );
      }
      return new Error(
        'Connection to authentication service failed. This might be due to network restrictions. Try Demo Mode to explore the app.',
      );

    case 'auth/operation-not-allowed':
      return new Error(
        'Email/password authentication is not enabled. Please try Demo Mode to explore all features.',
      );

    case 'auth/too-many-requests':
      return new Error(
        'Too many failed attempts. Please wait a few minutes before trying again, or use Demo Mode.',
      );

    case 'auth/user-disabled':
      return new Error(
        'This account has been disabled. Please contact support or try Demo Mode.',
      );

    case 'auth/user-not-found':
      return new Error(
        'No account found with this email address. Try creating an account or use Demo Mode.',
      );

    case 'auth/wrong-password':
      return new Error(
        'Incorrect password. Please try again or use Demo Mode to explore the app.',
      );
    ...
    case 'auth/timeout':
      return new Error(
        'Request timed out. Please check your connection or try Demo Mode.',
      );

    default:
      return new Error(
        'Authentication failed. Please try Demo Mode to explore the app without an account.',
      );
  }
};
...
const createDemoUser = async (): Promise<User> => {
  try {
    ...
    if (error.message === 'demo-offline') {
      ...
      throw new Error(
        'Demo mode is available offline! You can explore all features without an internet connection.',
      );
    }
    throw new Error(
      'Demo mode is temporarily unavailable. Please check your internet connection.',
    );
  }
};
  ...
  signUp: async (email: string, password: string) => {
    ...
    try {
      ...
      if (authError.code === 'auth/operation-not-allowed') {
        throw new Error(
          'Email registration is currently not available. Try Demo Mode to explore all features instantly!',
        );
      }

      if (authError.code === 'auth/network-request-failed') {
        throw new Error(
          'Unable to create account due to network issues. Try Demo Mode to start using the app immediately!',
        );
      }
...
export const firestoreService = {
  collection: (path: string) => {
    try {
      ...
      throw new Error(
        'Database connection failed. Your notes may not sync properly.',
      );
    }
  },
